...
- **Capture** — happens automatically WHILE a session runs and again after it ends: the `vault-miner` user unit polls every live session's transcript and mines its unswept tail once that tail passes ~32 KB; SessionEnd launches a detached headless worker that mines the unswept tail (1.7.0 default; the Stop hook is silent and only writes crash insurance, so nothing runs on your way out). Force an in-session sweep anytime with `/vault-update`, or set `VAULT_STOP_CAPTURE=1` for the pre-1.7 in-session nag every 30 min. The skill applies a *salience gate*: it only writes durable, non-obvious knowledge (the *why*, the gotcha, the cross-repo synthesis) — never what's trivially re-derivable from one source file.
